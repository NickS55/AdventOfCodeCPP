...
template <int SIZE, template <typename> typename TEST> class Conway2D {
public:
  ...
  bool get(int x, int y) const { return curr_grid()[x][y]; }

  void set(int x, int y) { curr_grid()[x][y] = true; }
...
template <typename GRID> struct Conway2DTest {
...
template <typename GRID> struct Conway2DTestHex {
  bool operator()(const GRID &grid, int x, int y) {
