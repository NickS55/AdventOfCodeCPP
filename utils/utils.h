...
// Split a delimited string of tokens into a vector of string tokens. Trims
// substrings by default and drops trimmed tokens which are empty by default.
// Not convinced how useful the option are, but you never know.
std::vector<std::string> split(std::string source, std::string delim,
                               Blanks allow_blanks = Blanks::Suppress,
                               Trim trim_subs = Trim::Yes);

// Read all lines from a file into a vector of strings. Trims lines by default
// and drops empty lines by default. Keeping empty lines can be useful when the
// blank lines in the input are meaningful.
std::vector<std::string> read_lines(std::istream &is,
                                    Blanks allow_blanks = Blanks::Suppress,
                                    Trim trim_lines = Trim::Yes);
std::vector<std::string> read_lines(std::string filename,
                                    Blanks allow_blanks = Blanks::Suppress,
                                    Trim trim_lines = Trim::Yes);

// Calls read_lines() with Trim::Yes and Blanks::Allow, then partitions the
// results into a vector of strings for each group.
...
// Convert a list of delimited values into an array. Assumes all values are of
// the same type. "1,2,3" => vector<int>{1, 2, 3}.
template <typename T>
...
std::tuple<Args...> parse_line(const std::regex &re, const std::string &str,
                               bool &matched, const std::string &delim = ";") {
  ...
  if (matched) {
    for (int i = 1; i < matches.size(); ++i) {
      ...
      // hard enough already). The delimiter can be used later to split the
      // string.
  ...
  std::apply([&ss](Args &...args) { ((ss >> args), ...); }, res);
  return res;
}
...
template <class... Args>
std::vector<std::tuple<Args...>> read_lines(std::string filename,
                                            std::string pat,
                                            const std::string &delim = ";") {
...
template <typename T, typename U> void check_result(T value, U expected) {
...
template <typename T> struct AlwaysTrue {
  bool operator()(T) const { return true; }
};

// Used for a default transform below
template <typename T> struct NullTransform {
...
//    auto x = comprehend(s, [](auto c) -> std::size_t { return c; }, [](auto c)
//    { return c % 10 == 7; });
...
vector<string> make_grid_with_border(const vector<string> &input, size_t width,
                                     char value);
vector<string> read_grid_with_border(std::string filename, size_t width,
                                     char value);
...
public:
  ...
  ~timer() {
    ...
    if (m_label.size() > 0)
      // std::cout << "Time elapsed: " << m_label << ": " << (diff.count() *
      // 1000) << "ms\n";
      std::cout << "Time elapsed: " << m_label << ": "
                << (diff.count() * 1000000) << "us\n";
    else
...
template <typename T> T lcm(T a, T b) { return abs(a) * (abs(b) / gcd(a, b)); }
